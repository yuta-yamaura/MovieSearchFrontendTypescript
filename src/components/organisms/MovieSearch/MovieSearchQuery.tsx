...
export const MovieSearchQuery = () => {
  const { year: yearParam, query: queryParam } = useParams();
  ...
  const fetchSelectedQueryMovies = async (page: number) => {
    try {
      const response = await axios.get(
        `${
          import.meta.env.VITE_API_URL
        }/api/movies/?year=${yearParam}&query=${queryParam}&page=${page}`
      );
  ...
  useEffect(() => {
    setCurrentPage(1);
    fetchSelectedQueryMovies(1);
  }, [yearParam, queryParam]);

  // ページが変更された時の処理
  useEffect(() => {
    if (currentPage > 1) {
      fetchSelectedQueryMovies(currentPage);
    }
  }, [currentPage]);
  ...
  const handleYearChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
    ...
    navigate(`/${year}/${encodeURIComponent(queryParam || "")}`);
  };
